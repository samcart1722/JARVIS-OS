...
|---|---|---|---|---|
| Exact-key discovery is bounded, ordered, and uses the existing knowledge-read permission. | Preserve deterministic local behavior and avoid unrestricted enumeration. | Maximum 50 visible records, one lookahead row, zero-match success, no pagination or cognitive fallback. | Sprint 26 accepted contract and tests | Merged into canonical `master` at `54e04261933ab85dbe4b237e6f81037d508b4a1c`; release tag `sprint-26-complete` pending; Sprint 25.1 remains the latest completed tagged release until tagging |
