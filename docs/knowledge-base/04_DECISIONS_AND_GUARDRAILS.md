...
|---|---|---|---|---|
| Exact-key discovery is bounded, ordered, and uses the existing knowledge-read permission. | Preserve deterministic local behavior and avoid unrestricted enumeration. | Maximum 50 visible records, one lookahead row, zero-match success, no pagination or cognitive fallback. | Sprint 26 accepted contract and feature-tree tests | Implemented on an unmerged/untagged feature tree; Sprint 25.1 remains canonical |
