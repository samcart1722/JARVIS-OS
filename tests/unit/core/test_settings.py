...
MEMORY_GROUNDED_VARIABLE = "MEMORY_GROUNDED_RESPONSE_ENABLED"
...
def clear_memory_grounded_environment(monkeypatch) -> None:
    monkeypatch.delenv(MEMORY_GROUNDED_VARIABLE, raising=False)
...
def test_env_example_contains_safe_reasoning_default() -> None:
    ...
    assert "MEMORY_GROUNDED_RESPONSE_ENABLED=false" in contents
...
def test_grounded_response_is_disabled_and_independent_by_default(
    monkeypatch,
) -> None:
    clear_memory_grounded_environment(monkeypatch)
    clear_memory_update_environment(monkeypatch)
    clear_memory_retrieval_environment(monkeypatch)
    clear_memory_prompt_environment(monkeypatch)
    clear_reasoning_environment(monkeypatch)
    ...
    assert configured.MEMORY_GROUNDED_RESPONSE_ENABLED is False
    assert configured.MEMORY_UPDATE_ENABLED is False
    assert configured.MEMORY_RETRIEVAL_ENABLED is False
    assert configured.MEMORY_PROMPT_CONTEXT_ENABLED is False
    assert configured.REASONING_ENABLED is False
...
def test_grounded_response_supports_boolean_override(
    monkeypatch,
    value: str,
    expected: bool,
) -> None:
    clear_memory_grounded_environment(monkeypatch)
    monkeypatch.setenv(MEMORY_GROUNDED_VARIABLE, value)

    assert (
        Settings(_env_file=None).MEMORY_GROUNDED_RESPONSE_ENABLED
        is expected
    )
